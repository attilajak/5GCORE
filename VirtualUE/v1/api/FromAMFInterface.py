...
class AMFSIDE(Resource):
	...
	def post(self):
		...
		if operator.eq(args['status'],"PDUSessionEstabilishmentReqAccept"):
			...
			variables.imsi = args['imsi']
			variables.UPFURI = args['UPFURI']
			variables.UEIP = args['AllocatedUEIp']
			variables.CNTunnelID = args['CNTunnelID']
			print(CurrentPath+":32   [VM][INFO]   "+"config enp0s8 ip")
			p = subprocess.Popen('sudo ifconfig enp0s8 192.168.2.120/24',shell=True)
			#MsgNum = 100
			#MsgNum = 100
			#MsgNum = 100
			#MsgNum = 100
